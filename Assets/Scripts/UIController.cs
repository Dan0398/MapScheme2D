...
public class UIController : MonoBehaviour
{
    ...
    MaskableGraphic LastPickedButton;
    float DeselectTimer;
    ...
    {
        if (LastPickedButton != null) LastPickedButton.color = Color.white;
        if (LastPickedButton == PickedButton || PickedButton == null)
        {
        ...
        {
            LastPickedButton = PickedButton;
            LastPickedButton.color = Color.yellow;
        }
    }
    ...
    void ApplyEditMode()
    {
        ...
        for (int i = 0; i < Results.Count; i++)
        {
            if (Results[i].gameObject.CompareTag("MapObject"))
            {
                ...
                {
                    DeselectPrevious();
                    SelectNewItemOnScene(Results[i].gameObject);
                }
                else
                {
        ...
        if (hit.collider != null && hit.collider.CompareTag("MapObject"))
        {
            ...
            {
                DeselectPrevious();
                SelectNewItemOnScene(hit.collider.gameObject);
            }
            else 
            {
                ModifyLastPickedObject();
    ...
    void SelectNewItemOnScene(GameObject NewItem)
    {    
        LastPickedItemOnScene = NewItem;
        LastPickedItemOnScene.GetComponent<MaskableGraphic>().material = HighlightedObjectMaterial;
        DeselectTimer = 0.5f;
    }
    ...
    void ModifyLastPickedObject()
    {
        ...
        if (Container != null)
        {
            var ItemType = Container.DataReference.GetType();
            foreach (var Category in Categories)
            {
                if (ItemType == Category.ControlledType)
                {
