...
public class MapIO
{
    ...
    public async Task<bool> TryLoadMap()
    {
        UnityEngine.EventSystems.EventSystem eventSystem = UnityEngine.EventSystems.EventSystem.current;
        eventSystem.gameObject.SetActive(false);
        SimpleFileBrowser.FileBrowser.ShowLoadDialog((string[] path) => { Debug.Log(path[0]); LastUsedPath = path[0]; }, () => { }, SimpleFileBrowser.FileBrowser.PickMode.Files, false, LastUsedPath);
        while (SimpleFileBrowser.FileBrowser.IsOpen) await Task.Yield();
        eventSystem.gameObject.SetActive(true);
